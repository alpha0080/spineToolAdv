               ...
               # print 'slotColorKeyFrameList_____4',slotColorKeyFrameList
                for i in range(0,len(slotColorKeyFrameList)) :
                    ...
                    if f == 0.0:
                      ...
                      #  vis = cmds.getAttr('%s.visibility'%boneName,t=0.0)
                      #  if vis == True:
                       #     visValue = 0.0
                      ##  else:
                          #  visValue = 1.0
                       ...
                       # for attr in slotColorAttr:
                        ...
                        alphaF = slotAlphaF * fadeF #* visValue
                       ...
                       # tempSlorColordTdict.update({"time":float('{:.4f}'.format(float(f)/fps)),"color":exportColorHexF})
                        ...
                        if visCount == 0:
                            ...
                            if f in visibilityList:
                              #  visTempList
                                indexOfVislist = visibilityList.index(f)
                               # print 'visibilityList_____________1',visibilityList,indexOfVislist
                                vis = cmds.getAttr('%s.visibility'%boneName,t=f)
                               # print 'vis____________________2',vis
                                if indexOfVislist == 0:
                                    pff = slotColorKeyFrameList[1]
                                    visPF = cmds.getAttr('%s.visibility'%boneName,t=f)
                                else:
                                    pff = f -0.01
                               ...
                               # print 'f',f,'pf',pf,'visPF',visPF
                                ...
                                #print 'f',f,'pf',pf,'visPF',visPF,'visValue',visValue,'visPFValue',visPFValue
                                    
                                
                                alphaF = slotAlpha * fade * visValue
                                alphaPF = slotAlpha * fade * visPFValue
                                alphaHex = "%02x"%int((alphaF/1)*255)  
                                exportColorHexF = str(colorHex + alphaHex) 
                                alphaPFHex = "%02x"%int((alphaPF/1)*255)  
                                exportPFColorHexF = str(colorHex + alphaPFHex)   
                              #  print 'pff', pff    ,float('{:.4f}'.format(float(pff)/fps)),  float('{:.4f}'.format(float(f)/fps))   
                                tempSlorColordTdict = {}   
                                             
                                tempSlorColordTdict.update({"time":float('{:.4f}'.format(float(pff)/fps)),"color":exportPFColorHexF})
                                tempSlorColordTdict.update({"time":float('{:.4f}'.format(float(pff)/fps)),"dark":exportDarkHexF})
                                slotTimeLineDict[slotName]["color"].append(tempSlorColordTdict)  
                             #   print 'exportTime______1'
                                tempSlorColordTdict = {}      
                                tempSlorColordTdict.update({"time":float('{:.4f}'.format(float(f)/fps)),"color":exportColorHexF})
                                tempSlorColordTdict.update({"time":float('{:.4f}'.format(float(f)/fps)),"dark":exportDarkHexF})
                                
                              #  print 'exportTime______2'

                             #   tempSlorColordTdict.update({"time":float('{:.3f}'.format(float(pff)/fps)),"dark":exportDarkHexF})
                               ## print 'exportTime______3'

                               # tempSlorColordTdict.update({"time":float('{:.3f}'.format(float(f)/fps)),"dark":exportDarkHexF})
                               # print 'exportTime______4'

                                slotTimeLineDict[slotName]["color"].append(tempSlorColordTdict)  
                        
                            else:
                                if f == slotColorKeyFrameList[1]:
                                    pass
                                else:
                                    tempSlorColordTdict.update({"time":float('{:.4f}'.format(float(f)/fps)),"color":exportColorHexF})
