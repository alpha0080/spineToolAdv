...
#cmds.cutKey( 'bone_star_A02_18', time=(0,3),option="keys" )                            
    ...
    def exortTOSpineJson(self): #regionSlot
        ...
        if  len(characterSetGrp) > 0: 
            ...
            if len(animLayerList) == 1:
                self.defineAllAnimLayerDict(i,slotList,allBoneList,allMeshSlotList,characterSetGrp)  
            else:
                for i in animLayerList:
                    
                    for j in animLayerList:
                        cmds.animLayer( j, e=True, selected=False,s=False)
                    cmds.animLayer( i, e=True, selected=True,s=True)
                    self.defineAllAnimLayerDict(i,slotList,allBoneList,allMeshSlotList,characterSetGrp)
             
              
                
         #   slotRegionTimeLineDict = self.defineRegionSlotAnimation(slotList)
          #  boneTimeLineDict = self.defineBoneAnimation(allBoneList)    
         ...
         #   animationDict['default']['bones'].update(chaSetTimelineDict)    
            exportData = {"skeleton":{"images": "../images/"},"bones":boneList,"slots":slotList,"skins":skinDict,"animations":{}}
            writeData = json.dumps(exportData, sort_keys=True , indent =4) 
            with open(fileName, 'w') as the_file:
                the_file.write(writeData)
        #return [{"skeleton":{"images": "../images/"},"bones":boneList,"slots":slotList,"skins":skinDict},regionSlotList,allBoneList]
    ...
    def defineBoneAnimation(self,allBoneList):
